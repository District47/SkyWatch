        ...
        fetch('/api/noaa/satellites')
            ...
            .then(function(sats) {
                ...
                sats.forEach(function(s) {
                    var id = 'sat-' + s.norad_id;
                    activeIds.add(id);

                    var icon = L.divIcon({
                        html: '<div class="sat-marker' + (s.visible ? ' sat-visible' : '') + '">&#x1f6f0;</div>',
                        className: '',
                        iconSize: [28, 28],
                        iconAnchor: [14, 14],
                    });
                    ...
                    if (satMarkers[id]) {
                        satMarkers[id].setLatLng([s.lat, s.lon]);
                        satMarkers[id].setIcon(icon);
                    } else {
                        satMarkers[id] = L.marker([s.lat, s.lon], { icon: icon, zIndexOffset: 1000 })
                            .addTo(map);
                    }

                    var tooltip = s.name + '\\n' +
                        'Alt: ' + Math.round(s.alt) + ' km\\n' +
                        'Vel: ' + s.velocity.toFixed(1) + ' km/s\\n' +
                        s.frequency.toFixed(4) + ' MHz';
                    if (s.visible) {
                        tooltip += '\\nEL: ' + s.elevation.toFixed(1) + '° AZ: ' + s.azimuth.toFixed(0) + '°';
                    }
                    satMarkers[id].bindTooltip(tooltip, { permanent: false, direction: 'top' });

                    // Draw footprint circle
                    if (satFootprints[id]) {
                        satFootprints[id].setLatLng([s.lat, s.lon]);
                        satFootprints[id].setRadius(s.footprint * 1000 / 2);
                    } else {
                        satFootprints[id] = L.circle([s.lat, s.lon], {
                            radius: s.footprint * 1000 / 2,
                            color: s.visible ? '#4fc3f7' : '#1e3a5f',
                            fillColor: s.visible ? '#4fc3f7' : '#1e3a5f',
                            fillOpacity: 0.05,
                            weight: 1,
                            dashArray: '4,4',
                        }).addTo(map);
                    }
                    // Update footprint color based on visibility
                    satFootprints[id].setStyle({
                        color: s.visible ? '#4fc3f7' : '#1e3a5f',
                        fillColor: s.visible ? '#4fc3f7' : '#1e3a5f',
                    });
                });
                ...
                Object.keys(satMarkers).forEach(function(id) {
                    if (!activeIds.has(id)) {
                        ...
                        if (satFootprints[id]) {
                            map.removeLayer(satFootprints[id]);
                            delete satFootprints[id];
                        }
                    }
                });
            })
